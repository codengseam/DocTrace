...
### 最近 10 条沉淀（按日期倒序）
- [2026-06-29 中国礼仪课专栏深度质检二轮闭环 95.6→97.5](#L1606)
- [2026-06-28 中国礼仪课专栏 + 多 agent 并行生成 + 三视角评审闭环](#L1552)
- [2026-06-27 资治通鉴续写至 77 章 + 提示词/引用沉淀 - 覆盖完整 1362 年通鉴](#L1513)
- [2026-06-28 富贵病手册新增「当代人的习惯账本」章节 - 章节插入与已有方法论复用](#L1417)
- [2026-06-28 现代人富贵病防治手册专栏生成 - 多 subagent 并行 + 医学术语中文化策略](#L1363)
- [2026-06-28 提交信息必须用中文且准确概括当前修改](#L1318)
...
|---|---|---|
| `book_structure`（双源同步 3 次 / 排序校验已固化 / 命名去重反复 / 跨章 chapter_sort 盲区 / 资治通鉴卷次核验） | 13 | no（双源同步未入） |
| `reader_interaction`（沉浸模式反复修复 4 次 / 防横屏已入 / Hero 移动端瘦身） | 11 | no（双源同步未入） |
| `content_quality`（质检规则分化 / 灵魂注入 / 标题评分 / 写作规则分工 / 医学术语中文化策略 / 续写约束沉淀 / 礼仪课 AI 幻觉核查） | 9 | no |
| `git_hygiene`（推送前 fetch / 冲突标记 / commit 覆盖 / 中文提交信息） | 5 | no（fetch/commit/提交信息未入） |
| `deployment`（SW CACHE_NAME 升级 / .nojekyll / 无 PyYAML 解析器兜底） | 4 | no（SW 版本未入） |
| `soul_injection`（灵魂注入 / 章回体标题 / 边链裁剪） | 3 | no |
| `ai_course`（专栏批量生成 / subagent 结果丢失 / 富贵病手册 5+3 两波并行 / 无 API 时并行写作路径 / 礼仪课 4+4+3 并行流水线） | 5 | no |
