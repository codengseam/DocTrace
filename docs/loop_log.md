...
## 易经课专栏生成沉淀（2026-06-23）

### 任务概述
为普通人、孩子和易经初学者创作 81 章 Markdown 易经学习专栏。核心目标：讲透每卦的「神」、用真实历史故事贴卦贴爻、引用孔子及历代名家、简单科普占卜、给现代人方向与力量。偏小说风格、阅读性强，厉害典故用「原文＋注释」方式讲。

### 交付物
- `/workspace/output/易经课/`：81 章 Markdown + `_meta.yaml`
  - 模块0开篇3章 + 模块1基础4章 + 模块2上经30卦 + 模块3下经34卦 + 模块4易传3章 + 模块5应用3章 + 模块6占卜3章 + 模块7总结1章
- `/workspace/docs/reviews/yijing_plan_20260623.md`：章节方案 v2（含 64 卦故事预配表、参考注家书目、9 条生成约束）
- `/workspace/docs/reviews/yijing_gen_spec.md`：共享生成规范（七字段 front matter、五段结构、字数、语言风格、禁止项、引用规范、史实要求、名家引用、核心典故讲法）

### 工作流（dev-workflow 五步法）
1. **重述需求**：一句话重述用户意图，确认核心目标
2. **生成计划并等确认**：输出 81 章清单 + 故事预配表，等用户确认
3. **执行**：并行 subagent 分 16 批生成，每批 5 章并行
4. **自检**：4 个并行质检 subagent 分模块抽检，按四维度打分，修复 P0
5. **沉淀**：本条记录

### 关键经验与共性问题

#### 1. 跨章重复控制（P1 可读性）
- **问题**：同一历史人物/事件在多卦出现时容易大段复述（如勾践在需卦展开，他卦又展开；文王在乾卦与明夷卦重复）。
- **解决**：在故事预配表中为每卦指定唯一贴卦故事，subagent 任务模板中明确「与XX卦错开角度，前事一笔带过」。
- **沉淀**：未来生成多章节专栏时，**预配表 + 错开角度指令**是控制跨章重复的有效组合。

#### 2. 史实层累交代（P0 真实性）
- **问题**：河图洛书数图、先天后天八卦、太极图均为宋代所传，易被误当上古真传；文王演易为传统说法；勾践"卧薪"为后起成语。
- **解决**：在生成规范中明确列出层累交代清单，subagent 任务模板中逐项提示。
- **沉淀**：**层累交代清单应作为生成规范的固定条目**，不可依赖 subagent 自觉。

#### 3. 名家点评编造风险（P0 真实性）
- **问题**：subagent 倾向于为每位古代注家配上"直接引语"，但部分引语实为编造或张冠李戴（如朱熹"易本卜筮之书也"实出《朱子语类》非《周易本义·序》；王弼"得意忘象"原文为"得意在忘象，得象在忘言"）。
- **解决**：质检阶段重点核查名家引文出处与原文；不确定的引语改为"大意据《XXX》"转述。
- **沉淀**：**名家引文须忠于原文，记不清宁可不引或转述**，不可硬配引语。这一条已写入 rules.md，但 subagent 执行时仍会越界，需质检兜底。

#### 4. 并行 subagent 风格一致性
- **问题**：81 章由多个 subagent 并行生成，若无共享规范，风格易发散。
- **解决**：建立 `yijing_gen_spec.md` 共享生成规范，所有 subagent 任务模板强制要求先读规范再动笔。
- **沉淀**：**共享生成规范文件是多 subagent 并行写作的必备基础设施**。

#### 5. 质检并行分块策略
- **问题**：81 章一次性质检上下文过长。
- **解决**：按模块分 5 个质检 subagent（上经前15/上经后15/下经前17/下经后17/剩余17），每个 subagent 只修复 P0，不做风格性改动。
- **沉淀**：**质检分块 + 只修 P0** 是大规模内容质检的可行模式，避免质检 subagent 越界改风格。
...
- 优秀数（≥90）：81
- 平均分：约 95
- P0 问题修复：12 类，涉及约 15 个文件
  - 上经后15卦：随卦"关中"→"关西"、大过卦悼公年龄矛盾
  - 下经前17卦：韩信之死地点"未央宫"→"长乐宫钟室"（3章）
  - 下经后17卦：丰卦康熙亲政年份
  - 剩余17章：朱熹出处张冠李戴、王弼引文不一致、编造名家点评（5处）、张冠李戴出处

### 规则/规范更新建议
1. `content-quality.md` 可补充：「名家引文须忠于原文，不确定时改为大意转述并标注出处」作为 P0 核查项。
2. `dev-workflow.md` 可补充：「多章节专栏生成时，须先建故事预配表与共享生成规范」。
3. `content-checklist.md` 名家点评项可细化：「核查引文是否为原话，非原话须标『大意据』」。

### 可复用资产
- `yijing_gen_spec.md` 可作为其他多章节专栏的共享生成规范模板。
- `yijing_plan_20260623.md` 中的「64 卦故事预配表」可作为「跨章重复控制」的范例。
- 并行 subagent 任务模板（含必读规范文件 + 章节信息 + 贴卦故事 + 卦辞爻辞 + 名家解析 + 写作要求 + 输出）可作为标准化写作任务模板。
